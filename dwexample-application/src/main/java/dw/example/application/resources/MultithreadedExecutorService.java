...
import dw.example.client.ServiceClient;
...
import dw.example.api.MultithreadedExecutorResponse;
...
public class MultithreadedExecutorService {
	...
	private ServiceClient serviceClient = new ServiceClient("http://localhost:8080/app/");
	...
	public Response runwithExecutorPost(@PathParam("count") int count) throws Exception{
		//Collect the futures in a list. Think about the memory impact if it's Integer.
		List<ListenableFuture<String>> futures = new ArrayList<>();
		for (int i=0; i<count; i++){
			Integer id = new Integer(i);
			ListenableFuture<String> future = listeningExecutorService.submit(() -> {
				CustomResponse response = serviceClient.sayGreetingsGet("hello","en");
				return "Response "+ response.getGreetings() + "and "+id+" by "+Thread.currentThread().getName();
			});
